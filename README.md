# 四合一樂透＋Gemini AI Cloudflare Worker版 V2.3.2

## 不用＠的搜尋指令

- 在已綁定群組輸入 `搜尋 今天台中天氣` 或 `搜尋 今天台灣重要新聞`，即可查詢並附上來源。
- 也接受 `收尋`、`搜索`，以及冒號分隔，例如 `搜尋：今天台灣新聞`。
- `搜尋圖片 美女`、`搜尋 一張美女圖` 等找圖片要求只回覆「圖片搜尋暫不支援，請自行到 Google 圖片搜尋。」，不呼叫 Gemini、不傳圖片。
- 其他一般聊天仍須真正＠機器人。原本四種樂透指令、群組限制和看使用者上傳圖片功能保留。
- 更新同名檔案後重新部署即可；沿用原本 Cloudflare 秘密設定和固定 Webhook 網址。不新增圖片 API。
